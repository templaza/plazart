...
class TZTemplateSocial {
    ...
    public function socialApiParser($embed_mode = false) {
         ...
         if(!($this->parent->API->get('fb_api_id', '') != '' && $this->parent->API->get('fb_login', '0') == 1)) {
              // clear FB login
            TZRules::setRule('/<plazart:fblogin(.*?)plazart:fblogin>/mis','');
         }
        else {
            TZRules::setRule('/<plazart:fblogin>/mi','');
            TZRules::setRule('/<\/plazart:fblogin>/mi','');
        }
    	...
    	if($catid == '' && $option == 'com_tz_portfolio' && $view == 'article' && $id != '') {
    		$db = JFactory::getDBO();
    	...
    	if($this->parent->API->get('fb_like', '0') == 1 && !$is_excluded) {
    		...
    		TZRules::setRule('/TZ_FB_LIKE_SETTINGS/',$fb_like_attributes);
    	} else {
    		// clear FB like
    		TZRules::setRule('/<plazart:social><fb:like(.*?)fb:like><\/plazart:social>/mi','');
    	}
        // G+
    	if($this->parent->API->get('google_plus', '1') == 1 && !$is_excluded) {
            ...
            if($this->parent->API->get('google_plus_share', 0)) {
                TZRules::setRule('/TZ_GOOGLE_PLUS_SHARE_SETTINGS="(.*?)"/','<g:plus action="share" href="${1}"></g:plus>');
            } else {
                TZRules::setRule('/TZ_GOOGLE_PLUS_SHARE_SETTINGS="(.*?)"/','');
            }
    		TZRules::setRule('/TZ_GOOGLE_PLUS_SETTINGS/',$google_plus_attributes);
    	} else {
    		// clear G+ button
    		TZRules::setRule('/<plazart:social><g:plusone(.*?)g:plusone><\/plazart:social>/mi','');
    	}
    	// Twitter
    	if($this->parent->API->get('tweet_btn', '0') == 1 && !$is_excluded && $option == 'com_tz_portfolio' && $view == 'article') {
    		...
    		TZRules::setRule('/TZ_TWEET_BTN_SETTINGS/',$tweet_btn_attributes);
    	} else {
    		// clear Twitter buttons
    		TZRules::setRule('/<plazart:social><a href="http:\/\/twitter.com\/share"(.*?)\/a><\/plazart:social>/mi','');
    	}
		// Pinterest
        if($this->parent->API->get('pinterest_btn', '0') == 1 && !$is_excluded && $option == 'com_tz_portfolio' && $view == 'article') {
              // configure Pinterest buttons               
              $pinterest_btn_attributes = $this->parent->API->get('pinterest_btn_style', 'horizontal');
              TZRules::setRule('/TZ_PINTEREST_SETTINGS/',$pinterest_btn_attributes);
         } else {
              // clear Pinterest button
              TZRules::setRule('/<plazart:social><a href="http:\/\/pinterest.com\/pin\/create\/button\/(.*?)\/a><\/plazart:social>/mi','');
         }
		
		
    	// check the excluded article IDs and category IDs
    	if(($option == 'com_tz_portfolio' && $view == 'article' && in_array($id, $excluded_articles, false)) ||
    		($catid != '' && $option == 'com_tz_portfolio' && $view == 'article' && in_array($catid, $excluded_categories, false)) || $embed_mode) {
    		$is_excluded = true;
            // clear SocialAPI div
    		TZRules::setRule('/<plazart:social(.*?)plazart:social>/mis','');
    		TZRules::setRule('/<plazart:socialAPI(.*?)plazart:socialAPI>/mis','');
    	} else {
            TZRules::setRule('/<plazart:social>/mi','');
            TZRules::setRule('/<\/plazart:social>/mi','');
            TZRules::setRule('/<plazart:socialAPI>/mi','');
            TZRules::setRule('/<\/plazart:socialAPI>/mi','');
        }
    	TZRules::setRule('/<meta name="og:/','<meta property="og:');
    }
