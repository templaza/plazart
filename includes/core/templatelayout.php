 ...
 * @package Plazart
 */
class PlazartTemplateLayout extends PlazartTemplate
{
	...
	*/
	public function getLayout () {
		return JFactory::getApplication()->input->getCmd ('plazartlayout', $this->_tpl->params->get('mainlayout'));
	}
	...
	*/
	function loadBlock($block, $vars = array())
	{
		...
		$path = PlazartPath::getPath ('layouts/system/'.$block.'.php');
		if(!$path){
		...
		echo isset($vars['spl']) ? $content : ("<div class=\"plazart-admin-layout-section\">".$content."</div>");
	} 
	...
	*
	* @param $block string
	*     Block name - the real block is layouts/blocks/[blockname].php
	*
	* @return string Block content
	*/
	function loadLayout($layout)
	{
		$path = PLAZART_TEMPLATE_PATH . '/layouts/'.$layout.'.php';
		if (!is_file ($path)) {
			$path = PLAZART_TEMPLATE_PATH . '/layouts/default.php';
		}
	...
	*/
	...
	protected function _parseJDoc ($matches) {
		...
		$tp = 'layouts/system/tp.php';
		$path = '';
		if (is_file (PLAZART_TEMPLATE_PATH . '/' . $tp)) {
			$path = PLAZART_TEMPLATE_PATH . '/' . $tp;
		} else if (is_file (PLAZART_PATH . '/' . $tp)) {
			$path = PLAZART_PATH . '/' . $tp;
		}
	...
	*/
	function addHead () {
		...
		$this->addCss ('plazart-admin-layout-preview', false); 

		// Add scripts
		$this->addScript (PLAZART_URL.'/bootstrap/js/jquery.js');
		$this->addScript (PLAZART_URL.'/bootstrap/js/bootstrap.js');
	}

	function megamenu($menutype){
		echo "<div class='plazart-admin-layout-pos block-nav plazart-admin-layout-uneditable'> <h3>Megamenu [$menutype]</h3></div>";
	}
