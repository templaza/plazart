 ...
 */
class JFormFieldPlazartDepend extends JFormField
{
	/**
	 * Element name
	 *
    ...
    protected function profile()
    {
        ...
        if (file_exists($xmlparams)) {
            /* For General Form */
            $plazartform = JForm::getInstance('jform', $xmlparams, array('control' => 'plazartform'));
		...
		if($level==1){
			$html = '
				<h4 rel="'.$level.'" class="block-head block-head-'.$class_name.' open '.$class.' " '.$group.' id="'.$regionID.'">
					...
					<a class="toggle-btn open" title="'.JText::_('Expand all').'" onclick="PlazartDepend.showseg(\''.$regionID.'\', \'level'.$level.'\'); return false;">'.JText::_('Expand all').'</a>
					<a class="toggle-btn close" title="'.JText::_('Collapse all').'" onclick="PlazartDepend.showseg(\''.$regionID.'\', \'level'.$level.'\'); return false;">'.JText::_('Collapse all').'</a>
		    	</h4>';
		} else {
			$html = '
				<h4 rel="'.$level.'" class="block-head block-head-'.$class_name.' open '.$class.' " '.$group.' id="'.$regionID.'">
					<span class="block-setting" >'.$_title.$_url.'</span> 
					<span class="icon-help editlinktip hasTip" title="'.htmlentities($_description).'">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
					<a class="toggle-btn" title="'.JText::_('Click here to expand or collapse').'" onclick="PlazartDepend.segment(\''.$regionID.'\', \'level'.$level.'\'); return false;">open</a>
		    	</h4>';
		} 
	...
	function group(){
		...
		if(!isset($matches[1]) || empty($matches[1])){
			preg_match_all('/plazartform\\[([^\]]*)\\]/', $this->name, $matches);
			$group_name = 'plazartform';
		}
	...
	function legend(){
		return '<legend class="plazart-admin-form-legend">' . JText::_($this->element['label']) . '<small class="plazart-admin-form-legend-desc">' . JText::_($this->element['description']) . '</small> </legend>';
	}
} 