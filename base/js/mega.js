...
;(function($){
	...
	var MegaMenu = function(elm, options){
		...
		$(this.menu).plazartimgload($.proxy(this.update, this, true));
	};
	...
	$.fn.megamenu = function (option) {
		return this.each(function () {
			...
			if (!data) {
				...
				PlazartMenuI.megamenu.push(data);
			} else {
	...
	$(document).ready(function(){
		...
		if(PlazartMenuI.megamenu || PlazartMenuI.megamenu.length){
			PlazartMenuI.mmid = null;
			$(window).on('resize.mm', function(){
				clearTimeout(PlazartMenuI.mmid);
				PlazartMenuI.mmid = setTimeout(function(){
					for(var i = 0, il = PlazartMenuI.megamenu.length; i < il; i++){
						PlazartMenuI.megamenu[i].update();
					}
