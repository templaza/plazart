...
less.refresh = function (reload) {
    ...
    /* Plazart framework */
    if(typeof PlazartTheme != 'undefined') {
        PlazartTheme.onCompile(0, less.sheets.length);
    }
    ...
    loadStyleSheets(function (e, root, _, sheet, env) {
        ...
        /* Plazart framework */
        if(typeof PlazartTheme != 'undefined') {
            PlazartTheme.onCompile(less.sheets.length - env.remaining, less.sheets.length);
        }
...
function createCSS(styles, sheet, lastModified) {
	...
	/* Plazart framework: add to the sheet position inteads of at the end of head */
...
function xhr(url, type, callback, errback) {
    /* Plazart framework: check if the file is loaded and store in cache */
    ...
    /* Plazart framework: end modified*/
    ...
    if (isFileProtocol && !less.fileAsync) {
        if (xhr.status === 0 || (xhr.status >= 200 && xhr.status < 300)) {
            /* Plazart framework: preprocess output before compile */
            var res = plazartPreprocess (xhr, url);
            callback(res.data);
        } else {
    ...
    function plazartPreprocess (xhr, url) {
        ...
        var fname = plazartFilename(url);
        if(window.PlazartTheme && PlazartTheme.others[fname] && url.indexOf('themes/' + PlazartTheme.theme + '/' + fname) == -1 && url.indexOf('plazart/base') == -1){
           res.data = res.data + "\n" + '@import "themes/' + PlazartTheme.theme + '/' + fname + '";' + "\n";
        }
        ...
        var vars = window.PlazartTheme ? PlazartTheme.vars : false,
            variables = '';
